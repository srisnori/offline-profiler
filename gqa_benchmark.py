...
def GQA_GPU(embed_dim, batch_size, seq_len, num_heads, num_kv_heads):
  ...
  with torch.no_grad():
    q = torch.rand(batch_size, num_heads, seq_len, head_dim, device="cuda", dtype=torch.float16)
    k = torch.rand(batch_size, num_kv_heads, seq_len, head_dim, device="cuda", dtype=torch.float16)
    v = torch.rand(batch_size, num_kv_heads, seq_len, head_dim, device="cuda", dtype=torch.float16)
